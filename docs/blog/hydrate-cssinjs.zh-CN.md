...
如果你研究过 EURO Design HUB 的官网，你会发现 EURO Design HUB 的组件并没有动态插入 `<style />` 来控制样式，而是通过 CSS 文件来控制样式：
...
在 [组件级别的 CSS-in-JS](/docs/blog/css-in-js) 一文中，我们提过。EURO Design HUB 的 Cache 机制并不需要计算出完整的样式。对于组件库而言，只要通过 Token 和 ComponentName 就可以确定生成样式一致性，所以我们可以提前计算出 hash 值：
...
CSS-in-JS 因为运行时的性能损耗而被人诟病。而在 EURO Design HUB 中，如果你的应用使用了 SSR，那么在客户端侧就可以直接跳过运行时生成样式的阶段从而提升性能。当然，我们会继续跟进 CSS-in-JS 的发展，为你带来更好的体验。
